...
def login(data: UploaderLoginRequest, db: Session = Depends(get_uploader_db)):
    ...
    print(f"  📥 Uploader User ID: '{data.user_id}'")
    ...
    if not user:
        print(f"  ❌ Step 1 (User ID): No user found with user_id = '{data.user_id}'")
    ...
    if user.phone == data.phone:
        print(f"  ✅ Step 3 (Phone Number): Match! Input '{data.phone}' == DB '{user.phone}'")
    else:
        print(f"  ❌ Step 3 (Phone Number): MISMATCH! Input '{data.phone}' != DB '{user.phone}'")
    ...
    print("-" * 60)
    if role_name == "UPLOADER" and user.phone == data.phone and pwd_match:
